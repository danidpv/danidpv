# 👋 ¡Hola! Soy Dani

### 💻 Fullstack Developer | Especialista en Frontend y Backend

Full Stack Developer · Automatización de procesos · Data & AI

Construyo aplicaciones, automatizaciones y soluciones de datos orientadas a resolver problemas reales.

Python · FastAPI · React · TypeScript · SQL Server · ClickHouse · Airflow · Docker · GitHub Actions
-

## 🚀 Tecnologías y Herramientas que Utilizo

### 🖥️ Frontend
- **React** ⚛️
- **Angular** 🅰️
- **TypeScript** 💙
- **JavaScript (ES6+)**

### 🛠️ Backend
- **Node.js**
- **Symfony (PHP)**

### 🗃️ Bases de Datos
- **MongoDB** 🍃
- **PostgreSQL** 🐘
- **DBeaver** (administración y consultas)
  
### 🔧 Testing y Herramientas
- **Postman** (pruebas de API)
- **Git** y **GitHub**
- **CI/CD** (integración y despliegue continuo)

### 🧠 Metodologías
- **Agile** y **Scrum** para una entrega continua y colaborativa

---

## 🧩 Qué hago

Desarrollo aplicaciones Full Stack con React, TypeScript, Python y FastAPI.
Automatizo procesos y flujos de datos.
Trabajo con SQL Server, ClickHouse y arquitecturas de datos.
Integro APIs, CI/CD y herramientas DevOps.
Interesado especialmente en automatización e Inteligencia Artificial.

---

## 🌟 Habilidades Destacadas

- Comunicación efectiva con equipos técnicos y no técnicos
- Adaptabilidad a nuevas tecnologías y retos
- Enfoque en **clean code**, arquitectura escalable y buenas prácticas
- Trabajo en entornos colaborativos
- Alto compromiso con la calidad del software y la satisfacción del cliente

---

## 📫 Conecta conmigo

- 🔗 [LinkedIn](https://www.linkedin.com/in/danidevelop)
- 🌐 [Portafolio Web](https://porfolio-2025-sigma.vercel.app/)

---

¡Gracias por visitar mi perfil! Si tienes una idea o proyecto en mente, **no dudes en contactarme**. 🚀
